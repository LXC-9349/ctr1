...
public class CommonUtils {
	 ...
	 * @throws ParseException
	 */
	public static String strParseDateFormStr(String source, String formatStr) {
		if (source == null)
			return null;
		// 先输出日期格式
		...
		try {
			return df2.format(df.parse(source));
	...
	/**
	 * 将字符串yyyyMMddHHmm格式化成yyyy-MM-dd HH:mm格式
	 * 
	 * @return
	 */
	public static String strParseDateFormat(String source) {
		if (source == null) {
			return null;
		}
		try {
			...
			timeStr.append(yyyy).append("-").append(MM).append("-").append(dd).append(" ").append(HH).append(":")
					.append(mm);
	 ...
	 * @return
	 */
	public static String todayHourMinute(Date date) {
		if (date == null) {
			return "";
		}
		try {
	 ...
	 * @return
	 */
	public static String secondConvertMin(int second) {
		try {
			int min = second / 60;
			int second1 = second % 60;
			StringBuilder minSecond = new StringBuilder();
			if (min < 10) {
				minSecond.append("0").append(min);
			} else {
				minSecond.append(min);
			}
			minSecond.append(":");
			if (second1 < 10) {
				minSecond.append("0").append(second1);
			} else {
				minSecond.append(second1);
			}
	 ...
	 * @return
	 */
	public static int sumMin(String hourMin) {
		if (StringUtils.isBlank(hourMin)) {
			return 0;
		}
		try {
			String[] hourMins = hourMin.split(":");
			if (hourMins.length >= 2 && StringUtils.isNotBlank(hourMins[0]) && StringUtils.isNotBlank(hourMins[1])) {
				int hour = CommonUtils.evalInt(hourMins[0]);
				int min = CommonUtils.evalInt(hourMins[1]);
				if (hour != -1000 && min != -1000) {
	 ...
	 * @return string
	 */
	public static String secToTime(Number s) {
		if (s == null) return "";
		...
		p += (minute < 10) ? ("0" + minute) : minute;
		p += ":";
		long second = seconds % 60;
		p += (second < 10) ? ("0" + second) : second;
		return p;
	};
	 ...
	 * @return
	 */
	public static long timeToSec(Time t){
		if(t == null) return 0;
		Calendar c = Calendar.getInstance();	
		c.setTime(t);
		long s = c.get(Calendar.HOUR_OF_DAY) * 3600 +  c.get(Calendar.MINUTE) * 60 +  c.get(Calendar.SECOND);		
		return s;
	}
	...
	public static String evalString(String src, String defaultValue) {
	...
	public static Integer getHoursDiff(String minuend, String subtrahend) {
		long between_days = 0L;
		try {
	...
	public static Integer getHoursDiffFromTomorrow() {
		return getHoursDiff(tomorrowToStr("yyyy-MM-dd 00:00:00"), todayToStr("yyyy-MM-dd HH:mm:ss"));
	}
	...
	public static boolean isSaturday(String source) {
		return isSaturday(parseDateFormStr(source, "yyyy-MM-dd HH:mm:ss"));
	}

	public static boolean isSunday(String source) {
		return isSunday(parseDateFormStr(source, "yyyy-MM-dd HH:mm:ss"));
	}

	public static boolean isHoliday(String source) {
		return isSunday(source) || isSaturday(source);
	}
	...
	public static double div(double d1, double d2) {
		...
		return divide.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
	}

	public static boolean existence(String ext) {
		...
		return list.contains(ext.toUpperCase());
	}

	/**
	 * 获取指定时间的mysql周期 如周三 java语言中为5，mysql为3
	 * 
	 * @param date
	 * @return
	 */
	public static int getWeekDayForMysql(Date date) {
		if (date == null)
			return -1;
	 ...
	 * @param endDate
	 * @param workingDays
	 *            工作日 如 0周一 1周二 2周三 3周四 4周五 5周六 6周日
	 * @return
	 */
	public static int countWorkingDays(Date beginDate, Date endDate, String workingDays) {
		...
		if (StringUtils.isNotBlank(workingDays)) {
			weekdays = Arrays.asList(CommonUtils.evalIntegerArray(StringUtils.split(workingDays, ",")));
		}
		int count = 0;
		for (;;) {
			int weekday = CommonUtils.getWeekDayForMysql(c.getTime());
	 ...
	 * @return
	 */
	public static Date getFirstWorkDayAgo(Date date, String workingDays){
	 ...
	 * @return
	 */
	public static boolean isWorkingDay(Date date, String workingDays) {
		if (date == null)
			return false;
		List<Integer> weekdays = null;
		if (StringUtils.isNotBlank(workingDays)) {
		...
		int weekday = CommonUtils.getWeekDayForMysql(date);
	...
	public static String convertPinyin(String chinese){
		...
		for (int i = 0; i < chars.length; i++) {
			if (chars[i] > 128) {
				try {
					result.append(PinyinHelper.toHanyuPinyinStringArray(
							chars[i], format)[0]);
	 ...
	 * @return
	 */
	public static boolean addZeroPhone(String phone){
		return !CommonUtils.isMatched("^(010|0[^10])\\d+", phone)
				&& CommonUtils.isMatched("^0((13[0-9])|(15[^4])|(18[0,1,2,3,4,5-9])|(17[0-8])|(19[0-9])|(147))\\d{8}$", phone);
	}
	...
	public static String getShortUrl(String longurl) {
		...
		try {
			...
			return URLDecoder.decode(jsonObj.getString("url_short"),"utf-8");
		} catch (Exception e) {
			return longurl;
		} finally {
			if(reader != null){
	...
	public static String mask(String str, Worker currentWorker) {
		if(StringUtils.isBlank(str)) {
			return str;
		}
		...
		int i = 0;
		//防止不知道的问题导致死循环
		while(m.matches() && i<10) {
			i++;
			String s = m.group(1);
			s = MemberUtils.maskPhone(s, currentWorker);
			str = str.replaceFirst("\\d{11,}", s);
			m = p.matcher(str);
		}
		return str;
	}
	...
	public static void main(String[] args) throws Exception {
		String reg = "^(http://)(.*?)(:\\d+)(/*)$";
		System.out.println("http://192.168.10.229:4434/".replaceFirst(reg, "$2"));
	}
