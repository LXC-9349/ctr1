...
public class PageMode {
    ...
    public PageMode(Integer page, Integer pageSize) {
        this.page = page;
    ...
    public void setSqlWhere(String sqlWhere) {
        if (this.sqlWhere.length() == 0){
            this.sqlWhere.append(" where 1=1 ").append(" and ").append(sqlWhere);
        }else {
    ...
    public <T> void setApiResult(ResponseData apiResult, List<T> resultMap) {
        ResponsePage<T> page = new ResponsePage<T>(getPage() <= 0 ? 1 : getPage(),getPageSize(), getTotal());
        page.setList(resultMap);
